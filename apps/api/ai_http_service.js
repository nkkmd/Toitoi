...
function createAiHttpService({ inspectionService }) {
  ...
  function handleRequest(request = {}) {
    ...
    if (method !== 'GET') {
      return buildJsonResponse(405, { message: 'Method not allowed' });
    }

    if (pathname === '/api/v1/ai/jobs') {
      return buildJsonResponse(200, {
        results: inspectionService.listJobs({
    ...
    if (jobMatch) {
      ...
      return job
        ? buildJsonResponse(200, job)
        : buildJsonResponse(404, { message: 'AI job not found', id });
    }

    if (pathname === '/api/v1/ai/annotations') {
      return buildJsonResponse(200, {
        results: inspectionService.listAnnotations({
          eventId: parsedUrl.searchParams.get('event_id'),
    ...
    if (annotationMatch) {
      ...
      return annotation
        ? buildJsonResponse(200, annotation)
        : buildJsonResponse(404, { message: 'AI annotation not found', id });
    }

    const eventMatch = pathname.match(/^\/api\/v1\/ai\/events\/([^/]+)$/);
    if (eventMatch) {
      return buildJsonResponse(200, inspectionService.getEventAiView(decodeURIComponent(eventMatch[1])));
    }
...
module.exports = { createAiHttpService };
