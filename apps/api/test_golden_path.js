...
function run() {
  ...
  assert.ok(translated.tags.some(tag => tag[0] === 'e' && tag[1] === GOLDEN_PATH_IDS.root));
  ...
  assert.ok(comparison.tags.some(tag => tag[0] === 'e' && tag[1] === GOLDEN_PATH_IDS.translated));

  console.log('PASS v0.2.0 golden path fixture preserves context, semantic relationships, and lineage references');
}
