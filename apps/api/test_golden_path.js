...
function assertFixtureContract(events) {
  ...
  assert.deepStrictEqual(
    findLineageTag(translated, GOLDEN_PATH_IDS.root),
  ...
  assert.deepStrictEqual(
    findLineageTag(comparison, GOLDEN_PATH_IDS.translated),
...
function run() {
  ...
  assert.strictEqual(ingestResult.duplicates.length, 0);
  ...
  assert.strictEqual(replayed.ingestResult.invalid.length, 0);
  ...
  assert.ok(rootCanonical.provenance);
