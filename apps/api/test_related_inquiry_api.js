...
const events = [
  {
    ...
    relationships: [{ relation: 'derived_from', target: 'explicit' }],
  },
  {
    id: 'explicit',
    type: 'inquiry',
...
const snapshot = { orderedIds: events.map(event => event.id), byId: Object.fromEntries(events.map(event => [event.id, event])) };
...
assert.strictEqual(response.body.results[0].id, 'explicit');
assert.strictEqual(response.body.results[0].classification, 'explicit_relation');
assert.strictEqual(response.body.results[1].id, 'lexical');
assert.strictEqual(response.body.results[1].classification, 'related_candidate');
