...
ローカルAIが Canonical Event を生成した後、**`packages/nostr/converter/canonical_to_nostr_converter.js` の `convertCanonicalToNostrDraft()` を通して Nostr Inquiry Event draft に変換し、そのまま transport する** 最小実装のコード例です。ここでは、Nostr のペイロードを手書きせず、Canonical Event を converter に渡してから署名・送信します。
...
} = require('../../packages/nostr/converter/canonical_to_nostr_converter');
