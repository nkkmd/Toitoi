...
この層は、農地固有の「生データ（センサー値・観察メモ）」を、まず **Canonical Event** として内部表現に正規化し、必要に応じて Nostr への transport projection として **『問い（Kind: 1042）』** を送信する役割を担います。
...
4.  **問いの二層構造（CANONICAL_EVENT.md / NOSTR_INQUIRY_SCHEMA.md 準拠）:**
    ...
    ```
    この流れの **converter と transport projection はエッジAIの責務** です。第1層は Canonical Event の `body.text` に相当する自然言語の主表現であり、農家・研究者・AIが各々の視点で読める「社会的インターフェース」です。第2層は Canonical Event の DSL projection として保持され、Nostr Inquiry Event へ写像される際に `dsl:*` タグ群へ展開されます。**DSLタグを持たない問いも完全に有効です。**
...
収集したコンテキストをLLM（ローカル推論エンジン、または必要に応じて外部APIのLLM）に渡し、まず Canonical Event の semantic core を組み立て、その後に Nostr への transport projection を行います。

ここでの責務分担は明確で、**エッジAIが Canonical Event を受け取り、converter を通して Nostr Inquiry Event に変換し、そのまま transport まで担う** という構成です。Relay / Sync / Archive はその結果を受け取る公開先であり、エッジAIの責務外です。
...
Edge AIは、タグ値を標準語彙（NOSTR_INQUIRY_SCHEMA.md / TOITOI_VOCABULARY.md 準拠）へ正規化するべきです。
...
ローカルのLLMからCanonical Event相当のJSONが出力された後、**エッジAIの converter が Nostr Inquiry Event に変換し、そのまま transport する** 最小実装のコード例です。DSL（第2層）・`dsl:confidence`（暫定）の付与を含む構成を示します。
...
// 2. LLMが生成した「問い」のデータ構造（内部的には Canonical Event を想定）
const inquiryPayload = {
    ...
    tags: [
        ...
        ["relationship", "weed_flora", "nutrient_cycle"],    // NOSTR_INQUIRY_SCHEMA.md の推奨ボキャブラリー
        ["phase", "intermediate"],
...
// 3. ローカル環境で暗号署名（Canonical Event → Nostr Inquiry Event の converter 後に生成される）
const signedEvent = finalizeEvent(inquiryPayload, secretKey);
console.log("署名済みイベントID:", signedEvent.id);

// 4. コモンズ・ネットワークへ transport（マルチパブリッシュ：3つ以上のリレーへ並列送信）
