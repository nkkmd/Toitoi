...
runtime replay は現状 unsupported ですが、将来的に file/archive-backed な実装へ拡張する余地を残します。

### `packages/lingonberry/`

Lingonberry の knowledge object / HTTP publish request を Toitoi の transport projection / ingest source として扱うための共有実装です。
Lingonberry upstream の `knowledge object` は protocol-native な canonical object として定義されていますが、Toitoi では内部中心モデルにせず、Canonical Event との adapter / converter 境界に閉じます。
...
| `DOCUMENTATION_UPDATE_POLICY.md` | ドキュメント更新情報ポリシー |
...
| `LINGONBERRY_STORAGE_AND_REPLAY.md` | Lingonberry storage / replay の運用メモ |
