...
この文書は、Nostr と Lingonberry を primary transport、ATProto を secondary transport として扱いながら、将来の transport 追加でも再利用できる保守的な identity policy を定義します。
...
現時点でこの policy が直接対象とする transport は次の 3 つです。

- Nostr
- Lingonberry
- ATProto

Lingonberry は Toitoi 内部中心モデルではなく、Canonical Event の transport projection / ingest source として扱います。
...
- `packages/lingonberry/adapter/test_ingest_pipeline.js`
- `packages/atproto/adapter/test_ingest_pipeline.js`
- `packages/nostr/storage/test_standard_api_views.js`
- `packages/lingonberry/storage/test_persistence.js`
- `packages/lingonberry/storage/test_replay.js`
...
- Nostr / Lingonberry / ATProto に、内容が似ているだけの event を 1 つずつ ingest する
