...
- 定義: 異なる文脈間で共有可能でありながら、完全には同一化されない対象です。
- 位置づけ: Toitoi では「問い」を boundary object として扱い、属地性を保ちながら他地域との翻訳を可能にする橋渡しとして設計します。
- 補足: 農業知識は気候、土壌、生態系、文化、実践に強く依存するため、ある地域の知識をそのまま別地域へ適用することはできません。Toitoi は、翻訳可能だが普遍化されない問いを共有することで、文脈を壊さずに知識を循環させます。
- 補足: 問いはコピー、再利用、再解釈、他地域への翻訳を経る可能性があり、その過程自体が知識生成の一部として扱われます。
- 補足: boundary object により、気候が異なる農地、異なる栽培方法、異なる文化圏のあいだでも、互いの文脈を破壊せず接続可能になることを目指します。
- 関連: [Inquiry Model](#inquiry-model), [provenance](#provenance), [lineage](#lineage), [canonical event](#canonical-event)
