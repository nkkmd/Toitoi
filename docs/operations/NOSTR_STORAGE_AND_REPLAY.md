...
- Nostr 内の raw duplicate はここで扱うが、cross-source の同一性判定はここでは決めない
...
- source を跨ぐ semantic duplicate の merge は、保守的に「明示的に同一といえる場合」に限定します。曖昧な case は別 event として保持し、必要なら relation / lineage でつなぎます。
