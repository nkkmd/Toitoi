...
corepack pnpm --filter @toitoi/lingonberry-transport test
corepack pnpm --filter @toitoi/nostr replay -- --protocol nostr --storage-dir /path/to/storage --verify
```

- `--filter` は「この package だけ」を指します
- `@toitoi/api` などは package 名です
- `--` の後ろは、その package の script に渡す引数です
- `@toitoi/nostr replay` は protocol-aware で、`--protocol nostr` だけでなく `--protocol lingonberry` / `--protocol atproto` も受けられます
...
- `@toitoi/lingonberry-transport`
  - Lingonberry archive / wire log ingest worker を起動するとき
- `@toitoi/atproto-transport`
  - ATProto JSONL / live ingest worker を起動するとき
- `@toitoi/nostr`
  - protocol-aware replay や Nostr storage 関連を使うとき
- `@toitoi/lingonberry`
  - Lingonberry adapter / converter / storage 関連を使うとき
- `@toitoi/atproto`
  - ATProto adapter / converter / storage 関連を使うとき
...
- [infra/transports/lingonberry/README.md](../../infra/transports/lingonberry/README.md)
- [infra/indexers/INDEXER_API_SETUP.md](../../infra/indexers/INDEXER_API_SETUP.md)
