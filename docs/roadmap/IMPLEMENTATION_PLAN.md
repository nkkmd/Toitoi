...
- 手元で 1 回だけ live smoke を実行する場合の手順:
  1. bot アカウントの実 PDS、handle、app password を用意する
     ...
     - `ATPROTO_PDS_HOST` は `bsky.social` ではなく、bot アカウントの DID document にある `service[].serviceEndpoint` を使う
     - handle は `com.atproto.identity.resolveHandle` で DID に解決し、DID document の `id: #atproto_pds` を見る
     - `https://<NAME>.<REGION>.host.bsky.network` は推測せず、DID document に書かれた `serviceEndpoint` をそのまま設定する
  2. 一時的に環境変数を設定する
     ```bash
     export ATPROTO_LIVE_SMOKE_TEST=1
     export ATPROTO_PDS_HOST=https://<bot-account-pds-host>
