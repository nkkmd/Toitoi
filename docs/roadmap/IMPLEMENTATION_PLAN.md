...
Phase 16 を完了として扱ったうえで、次の順序で進めます。

1. Lingonberry の一次情報を確認し、transport capability を固定する
2. Lingonberry transport schema と machine-readable schema を追加する
3. `packages/lingonberry/` の adapter / converter / storage を実データ ingest まで通す
4. Standard API の multi-transport 対応を確認する
5. 運用・移行・拡張の標準化を固める
