...
| 16 | 追加 protocol を実データで通す | skeleton ではなく実データ ingest まで通せる |
| 17 | registry 駆動の起動と選択をさらに整理する | protocol 選択を単純にできる |
...
## フェーズ 16: 追加 protocol 実データ ingest

### 目的

Phase 14 で整えた multi-transport の枠組みと、Phase 15 で固めた canonical identity / provenance 実装を使って、追加 protocol を 1 つ実データ ingest まで通し、skeleton ではない運用可能な状態にする。
...
- まだ着手前
- Phase 16 の追加 protocol を踏まえて詳細を詰める
...
1. canonical identity / provenance の実装を固める
