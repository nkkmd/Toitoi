...
関連: [Release Notes](./RELEASE_NOTES.md)
...
| 17 | Lingonberry transport を採用する | Nostr / ATProto と同じ責務粒度で実データ ingest まで通せる |
...
## フェーズ 17: Lingonberry transport 採用

### 目的

Phase 14 で整えた multi-transport の枠組みと、Phase 15/16 で固めた canonical identity / provenance / identity key 実装を使って、Lingonberry を Nostr / ATProto に続く Toitoi transport として採用する。

対象実装は [`nkkmd/lingonberry`](https://github.com/nkkmd/lingonberry) とする。Toitoi では Lingonberry を内部中心モデルではなく、Canonical Event の transport projection / ingest source として扱う。

このフェーズでは、単なる descriptor skeleton ではなく、schema、adapter、converter、storage、replay、Standard API 接続、運用文書までを、Nostr / ATProto と同じ責務粒度で揃える。

### 着手前確認

- `nkkmd/lingonberry` の README、実装、ライセンス、更新状況を確認する
- Lingonberry の event / message / record / storage model を整理する
- transport identity、author / owner、timestamp、dedupe key、ordering、trust の根拠を確認する
- delete / replace / update semantics があるか、ある場合は transport-local な判断として扱えるか確認する
- raw acquisition が live ingest、batch ingest、archive ingest のどれに向くか決める
...
- `docs/protocols/LINGONBERRY_TRANSPORT.md` を追加する
- `docs/protocols/LINGONBERRY_INQUIRY_SCHEMA.md` を追加する
- `schemas/lingonberry-inquiry.schema.json` を追加する
- `docs/protocols/INQUIRY_TRANSPORT_SCHEMA_TEMPLATE.md` との対応を確認する
- `docs/operations/LINGONBERRY_STORAGE_AND_REPLAY.md` を追加する
- `packages/lingonberry/README.md` と `infra/transports/lingonberry/README.md` を追加する

#### Package implementation

- `packages/lingonberry/protocol.js` で protocol descriptor と capability rows を定義する
- `packages/lingonberry/adapter/` で validate / verify / normalize / canonicalize / classify / ingest を実装する
- `packages/lingonberry/converter/` で canonical と Lingonberry transport draft の相互変換を実装する
- `packages/lingonberry/storage/` で raw / canonical append-only 保存、replay、derived index を実装する
- 必要に応じて `packages/lingonberry/live/` に gated live smoke helper を置く

#### Runtime / API / ops

- `packages/protocol/` の catalog / registry から Lingonberry descriptor を参照できるようにする
- `apps/api/server.js` の replay module / storage module 解決に `lingonberry` を追加する
- `TOITOI_PROTOCOL=lingonberry` と `TOITOI_TRANSPORT_SOURCES` で起動できることを確認する
- `infra/transports/lingonberry/` に ingest worker または batch ingest 入口を置く
- multi-transport replay で Nostr / ATProto / Lingonberry を同じ canonical view に流せることを確認する

#### Tests

- schema mirror test を追加する
- adapter / converter / ingest pipeline の unit test を追加する
- persistence / replay / indexer の test を追加する
- protocol descriptor の contract test を追加する
- Standard API から Lingonberry replay 結果を参照する test を追加する
- live 接続が必要な場合は環境変数で gated smoke test にし、通常 CI では実行しない

### 完了条件

- Lingonberry の transport schema と machine-readable schema がある
- Lingonberry の raw event / record / archive fixture と sample archive がある
- `packages/lingonberry/` に adapter / converter / storage / protocol descriptor がある
- Lingonberry が実データで ingest できる
- replay で derived index を再構築できる
- Standard API から canonical view として参照できる
- `TOITOI_PROTOCOL=lingonberry` で単体起動できる
- `TOITOI_TRANSPORT_SOURCES` で Nostr / ATProto と並べて扱える
- 既存 protocol の挙動を壊さない
- Nostr / ATProto と同じ capability table 粒度で Lingonberry の差分を説明できる
- Lingonberry の運用ガイドが protocol ごとに読める
...
- 2026-06-25 時点で Phase 18 を完了扱いとする
- `TOITOI_TRANSPORT_SOURCES` がある場合は multi-transport mode を優先し、単一 protocol selection より上位に置く方針を固定した
- 単一 protocol mode では `TOITOI_PROTOCOL` を使い、未指定なら default protocol として `nostr` を選ぶ方針を固定した
- registry 登録済みだが runtime replay が未対応の protocol と、registry に存在しない unknown protocol のエラーを区別した
- `/health` と `/api/v1/protocols` に storage runtime の説明を揃えて、API introspection と README / ops doc の説明を一致させた
- `packages/protocol/protocol_runtime.js` / `packages/protocol/protocol_storage_runtime.js` / `apps/api/server.js` / `apps/api/standard_api_service.js` と関連テストで確認した

---

## 当面の優先順位

Phase 18 までを v0.1.0 の release candidate 範囲として扱います。  
この時点の主眼は、production-grade な完成宣言ではなく、Canonical Event / Standard API / multi-transport MVP の最初の公開基準点を固定することです。

v0.1.0 タグ後の優先順位は次の通りです。

1. Lingonberry の実 carrier に対する手元 live smoke を行う
2. Lingonberry live relay ingest worker の必要性を確認する
3. Standard API と protocol registry の公開説明を利用者目線で磨く
4. LocalFS の file/archive-backed replay を実装するか、metadata-only のまま維持するかを判断する
5. 次の protocol / API 互換性変更を `0.1.x` で扱うか、後続 minor に送るかを整理する

Phase 13-18 で扱った multi-transport fan-out / fan-in、canonical identity、identity key / claim、registry 駆動の起動選択は、v0.1.0 時点では最小実装が接続済みの前提として扱います。

---

## 後回しにするもの

以下は重要ですが、v0.1.0 では固定しません。
...
追加 protocol の細かな federation ルールや transport 横断の高度な自動統合は、v0.1.x の運用で見えた実データの差分を踏まえて判断します。
