...
**Version: 0.6.6** | **Status: evolving** | **Last updated: 2026-06-24**
...
Lingonberry worker は、Lingonberry archive / wire log / JSONL を読み、canonicalized event を保存する運用 worker です。  
...
ここでは「Lingonberry worker は archive / wire log を単発 ingest できる」「定期回収は systemd timer で扱う」だけ押さえておけば十分です。
...
現時点では常時接続ではなく、Lingonberry archive / wire log を定期的に読み直す storage-only の ingest worker として扱います。
...
- 現時点の worker は Lingonberry archive / wire log / JSONL を batch ingest として読み、timer で継続運用する
