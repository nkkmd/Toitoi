...
1. `TOITOI_TRANSPORT_SOURCES` がある場合は single protocol selection より優先する
1. 単一 protocol mode では `TOITOI_PROTOCOL` で protocol を選ぶ
1. `TOITOI_PROTOCOL` が未指定なら default protocol として `nostr` を使う
1. `TOITOI_STORAGE_DIR` を指定して replay 由来の snapshot を読む
1. replay が無い protocol は、metadata は introspection に表示するが、storage 付き起動では明示エラーにする
...
`/health` と `/api/v1/protocols` には、選択中の storage runtime が含まれます。  
`/api/v1/protocols/:protocol` では、registry に登録された protocol ごとの replay 対応状況を確認できます。未知の protocol 名は `Unknown protocol`、registry 登録済みだが replay 未対応の protocol は `replayStorage not implemented` として区別します。
