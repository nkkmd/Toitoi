...
> **現行参照実装:** `apps/api/standard_api_service.js` と `apps/api/server.js` が canonical view を返す薄い service layer / HTTP entry です。Nostr 側の ingest は `infra/transports/nostr/relay_ingest_worker.js` が担い、`worker.js` という旧来のローカル実装名は歴史的表記として読むのが安全です。
...
│       └── toitoi-worker (relay_ingest_worker.js) ───────────────┘│
└───────────────────────────────────────────────────────────────┘
```
...
## 5. アプリケーションの実装（Canonical Event 版）

API と ingest worker のコードを `~/toitoi-indexer/` 配下に作成します。worker は raw event を Canonical Event の入力として扱い、ローカルのリレープロセス（ポート8008）へ直接通信するように設定します。

### Step 5.1: 取り込みワーカーの実装 (`relay_ingest_worker.js`)

```bash
mkdir -p ~/toitoi-indexer/apps/api
...
// ─────────────────────────────────────────────────────────────────
// saveEventToDB: raw event を canonicalized event の土台として保存する
//
...
//   - tagValue2: value_1（モデル名・変数名・起点変数など）
//
//   4要素タグ ["dsl:var", "m1", "microclimate", "independent"] のように
//   tagValue2 に収まらない value_2（変数ロール・終点変数）は、
//   同一の tagKey / model_id を持つ 2行目の Tag レコードとして保存する。
//   これにより完全な情報を保持しつつ、スキーマ変更を避けられる。
// ─────────────────────────────────────────────────────────────────
    ...
    await prisma.$transaction(async (tx) => {
        await tx.event.create({
            data: {
                ...
                createdAt: event.created_at, // Nostrのcreated_atをPrismaのcreatedAtにマッピング
                rawJson:   event,
            }
        });
        ...
        for (const tag of event.tags) {
            ...
            const tagKey    = tag[0];
            const tagValue1 = tag[1];
            const tagValue2 = tag[2] ?? null;
            const tagValue3 = tag[3] ?? null; // dsl:var のロール / dsl:rel の終点変数

            // 1行目: 通常通り tagValue2 まで格納
            await tx.tag.create({
                data: {
                    eventId:   event.id,
                    tagKey,
            ...
            // DSL 4要素タグの場合: value_2（tagValue3）を2行目として保存
            // 例: ["dsl:var", "m1", "microclimate", "independent"]
            //       → 1行目: tagValue1="m1", tagValue2="microclimate"
            //       → 2行目: tagValue1="m1", tagValue2="independent"
            // 例: ["dsl:rel", "m1", "microclimate", "weed_flora"]
            //       → 1行目: tagValue1="m1", tagValue2="microclimate"
            //       → 2行目: tagValue1="m1", tagValue2="weed_flora"
            if (tagKey.startsWith('dsl:') && tagValue3 !== null) {
                await tx.tag.create({
                    data: {
                        eventId:   event.id,
                        tagKey,
                        tagValue1, // model_id（同一）
                        tagValue2: tagValue3,
                    }
                });
            }
...
### Step 5.2: Standard API サーバーの実装 (`apps/api/server.js`)

> **注意：** 以下の実装例は、Canonical Event の canonical view を返す HTTP API の設計サンプルです。現行フェーズ5では、同等の参照・検索機能は `packages/nostr/storage/indexer.js` と `apps/api/standard_api_service.js` が担っています。DB 直結の内容は歴史的な参考実装として読み、現行運用では `apps/api/server.js` を中心に扱ってください。

```bash
nano ~/toitoi-indexer/apps/api/server.js
```
...
// 最新の canonicalized event 一覧を取得（ページネーション対応）
// クエリパラメータ:
...
//   dsl_model      : DSLモデル名で絞り込み（例: climate_model）        ※ 任意
//   dsl_var        : DSL変数名で絞り込み（例: microclimate）           ※ 任意
//   dsl_role       : DSL変数ロールで絞り込み（dsl_var と併用推奨）     ※ 任意
//                    値: independent / dependent / mediator / moderator
...
//     tagKey    = 'dsl:model' / 'dsl:var' / 'dsl:rel' / 'dsl:meta'
//     tagValue1 = model_id（例: "m1"）
//     tagValue2 = value_1（モデル名・変数名・起点変数）
//   dsl:var のロールや dsl:rel の終点変数は 2行目レコードとして保存されており、
//   EXISTS サブクエリを組み合わせることで絞り込みが可能。
//
// レスポンス:
...
app.get('/api/v1/inquiries/query', async (req, res) => {
    try {
        ...
        const hasAnyParam = [
            ...
            crop_family, relationship, phase,
            dsl_model, dsl_var, dsl_role,
            since, until,
        ].some(v => v !== undefined);
        ...
        if (dsl_model) {
            conditions.push(`
                EXISTS (
                    SELECT 1 FROM "Tag" t
                    WHERE t."eventId"   = e.id
                      AND t."tagKey"    = 'dsl:model'
                      AND t."tagValue2" = $${p}
                )
            `);
            values.push(dsl_model);
            p++;
        }

        //    【dsl_var】（単独指定: 変数名で絞り込み）
        //    【dsl_var + dsl_role】（組み合わせ: 変数名と役割の両方が存在するイベントを絞り込み）
        //
        //    格納方式の復習:
        //      1行目: tagKey='dsl:var', tagValue1=model_id, tagValue2=変数名
        //      2行目: tagKey='dsl:var', tagValue1=model_id, tagValue2=役割
        //    → 変数名と役割は別レコードに分かれているため、EXISTS を2つ AND で結合する。
        if (dsl_var && dsl_role) {
            // 変数名を持つレコードが存在し、かつ
            // 同一 model_id で役割レコードも存在するイベントを返す
            conditions.push(`
                EXISTS (
                    SELECT 1 FROM "Tag" t1
                    WHERE t1."eventId"   = e.id
                      AND t1."tagKey"    = 'dsl:var'
                      AND t1."tagValue2" = $${p}
                )
                AND EXISTS (
                    SELECT 1 FROM "Tag" t2
                    WHERE t2."eventId"   = e.id
                      AND t2."tagKey"    = 'dsl:var'
                      AND t2."tagValue2" = $${p + 1}
                )
            `);
            values.push(dsl_var, dsl_role);
            p += 2;
        } else if (dsl_var) {
            conditions.push(`
                EXISTS (
            ...
            // dsl_var なしで dsl_role のみ指定（役割だけで絞り込む）
            conditions.push(`
                EXISTS (
                    SELECT 1 FROM "Tag" t
                    WHERE t."eventId"   = e.id
...
// 指定した event ID をルートとし、lineage を再帰結合して
// N階層の子ノード（派生・結合された canonicalized event）をツリー構造の JSON で返す。
...
app.listen(3000, () => {
    console.log('🚀 Standard API Server running on port 3000');
});
```
...
PM2を使って Standard API と ingest worker をバックグラウンドで起動し、監視します。 `ecosystem.config.cjs`を`~/toitoi-indexer/` 直下に作成します。
