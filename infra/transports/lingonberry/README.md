...
現時点では Phase 17 の carrier / archive / JSONL ingest 入口として、`lingonberry_ingest_worker.js` を置いています。Nostr と同じく、worker は 1 回の取得と保存を行い、`systemd timer` で定期実行します。
...
- carrier からの直接取得は `GET /v1/objects` を使う
...
常時接続ではなく、carrier の object collection を一定間隔で取得する storage-only の運用です。
