...
function createAiWorker({ queue, provider, store = null, now = () => new Date().toISOString(), createId = defaultAnnotationId }) {
  if (!queue || typeof queue.claimNext !== 'function') {
    throw new TypeError('queue must implement claimNext');
  }
  if (!provider || typeof provider.infer !== 'function') {
    throw new TypeError('provider must implement infer');
  }
  ...
  async function runOnce() {
    ...
    try {
      ...
      if (store && typeof store.appendAnnotation === 'function') {
        store.appendAnnotation(annotation);
      }
